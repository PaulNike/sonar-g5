...
public class EmpresaServiceImpl implements EmpresaService {
    ...
    public ResponseEntity<BaseResponse<Empresa>> crear(EmpresaRequest request) {
        ...
        if(exist){
            ...
            response.setCode(Constants.CODE_EXIST);
            response.setMessage(Constants.MSJ_EXIST);
            response.setObjeto(Optional.empty());
            return ResponseEntity.ok(response);
        }else{
            Empresa empresaGuardar = empresaRepository.save(getEntity(request));
            BaseResponse<Empresa> response = new BaseResponse<>(Constants.CODE_OK,Constants.MSJ_OK, Optional.of(empresaGuardar));
            return ResponseEntity.ok(response);
        }
    }
